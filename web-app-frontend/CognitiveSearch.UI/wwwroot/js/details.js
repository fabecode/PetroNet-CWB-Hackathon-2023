...
function GetFileHTML(data, result) {
    ...
    var path = result.url; // direct path to blob with auth token
    ...
    if (path != null) {
        ...
        if (pathLower.includes(".pdf")) {
            ...
            pdfviewerpath = "https://drive.google.com/viewerng/viewer?embedded=true&url=" + path
            fileContainerHTML = `<a href="${path}" class="downloadbutton">Click to Download if PDF Previewer does not work</a><br><br><embed src="${pdfviewerpath}" type="application/pdf" width="1000px" height="2100px"=/>`
...
function GetTranscriptHTML(result) {
    ...
    if (result.merged_content) {
        if (result.merged_content.length > 0) {
            full_content = htmlDecode(result.merged_content.trim().replace(/[\r\n]+/g, " "));
        }
    }
    else
    {
        // otherwise, let's try getting the content -- although it won't have any image data.
        full_content = result.content.trim().replace(/[\r\n]+/g, " ");
    }
...
function GetMetadataHTML(result) {
    ...
    for (var key in result) {
        if (result.hasOwnProperty(key)) {
            if (key !== "content" && key !== "enriched" && key !== "id" && key !== "layoutText" && key !== "ImageTags" && key !== "ImageCaption" && key !== "text" && key !== "merged_content" && key !== "translated_text" && key != "metadata_storage_size" && key != "metadata_storage_path") {
                if (result[key] !== null) {
